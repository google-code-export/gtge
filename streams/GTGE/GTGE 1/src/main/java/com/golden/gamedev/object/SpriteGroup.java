...
public class SpriteGroup {
	...
	public void add(Sprite member) {
		...
		if (++this.size >= this.sprites.length) {
			...
			this.sprites = Utility.expand(this.sprites, this.expandFactor, true);
		}
	}
	
	/**
	 * Inserts sprite at specified index, range from [0 - {@linkplain #getSize() size}].
	 * <p>
	 * 
	 * Sprite at the first index (index = 0) is at the back of other sprites (first rendered). <br>
	 * Sprite at the last index (index = {@linkplain #getSize() size}-1) is rendered on top of other sprites (last
	 * rendered).
	 */
	 ...
	 * {@link com.golden.gamedev.object.Sprite#setImmutable(boolean) immutable sprite}.
	 * 
		...
		while (i < this.size) {
			...
			if (removeImmutable == false) {
				...
				while (i + removed < this.size
						&& (this.sprites[i + removed].isActive() == false && this.sprites[i + removed].isImmutable() == false)) {
					removed++;
				}
				
			} else {
				// remove all inactive sprites include immutable ones
				while (i + removed < this.size && this.sprites[i + removed].isActive() == false) {
					removed++;
				}
			}
	 ...
	 * <p>
	 * 
	 * This method only sort the sprites once called, use {@link #setComparator(Comparator)} instead to sort the sprites
	 * on each update.
	 * 
	 * @see #setComparator(Comparator)
	 */
	public void sort(Comparator<Sprite> c) {
	 ...
	 * <p>
	 * 
	 * This method usually used to check whether this group still have alive member or not. <br>
	 * Note: alive group has different meaning from {@linkplain #setActive(boolean) active} group, inactive group is not
	 * updated and rendered even there are many active sprites in the group, but dead group means there are no active
	 ...
	 * </pre>
	 * 
	 * @return The first found active sprite, or null if there is no active sprite in this group.
	 * @see com.golden.gamedev.object.Sprite#setActive(boolean)
	 */
	...
	/**
	 * Returns the first inactive sprite found in this group (the returned sprite is automatically set to active), or
	 * null if there is no inactive sprite, please see {@link com.golden.gamedev.object.Sprite#setImmutable(boolean)}
	 * for tag method of this method.
	 * <p>
	 * 
	 * This method is used for optimization, to reuse inactive sprite instead of making new sprite.
	 * <p>
	 * 
	 * For example :
	 * 
	 ...
	 * <p>
	 * 
	 * This method is only a convenient way to return the first found inactive sprite. To filter the inactive sprite,
	 * simply find and then filter the inactive sprite like this :
	 * 
	 * <pre>
	 *    SpriteGroup A_GROUP;
	 ...
	 * 
	 * 
	 * @return The first found inactive sprite, or null if there is no inactive sprite in this group.
	 * @see com.golden.gamedev.object.Sprite#setImmutable(boolean)
	 */
	...
	/**
	 * Sets allocation size for empty sprite (null sprite). This factor is used only for optimization (reduce the cost
	 * of array enlargement operation).
	 * <p>
	 * 
	 * The process : <br>
	 * If there is a new member insertion to the group and the group sprite array has been full, the array is expanded
	 * as large as this factor.
	 * 
	 * For example: <br>
	 ...
	 * <p>
	 * 
	 * If new member is added into this group, the group size is automatically grow to 120 (100+20). <br>
	 * The new sprite added is at index 101 and the rest is empty sprite (null sprite).
	 * <p>
	 * 
	 * <b>Note: use large expand factor if there are many sprites inserted into this group in a period.</b>
	 * 
	 * @see #getExpandFactor()
	 */
	...
	public String toString() {
		return super.toString() + " " + "[name=" + this.name + ", active=" + this.getSize() + ", total="
				+ this.sprites.length + ", member=" + this.getActiveSprite() + ", background=" + this.background + "]";
	}
