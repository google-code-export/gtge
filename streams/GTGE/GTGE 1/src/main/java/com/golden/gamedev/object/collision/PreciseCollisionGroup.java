...
/**
 * <p>
 * Subclass of <code>CollisionGroup</code> that calculates the precise positions of the <code>Sprite</code>s at the
 * moment of collision. It is suitable for collisions that need the colliding objects to stop rather than vanish.
 * </p>
 * 
 * <p>
 * For example:
 * <ul>
 * <li>Collisions between balls that need to stop or bounce precisely off each other.</li>
 ...
 * 
 * <p>
 * This class may not work as expected with concave sprites- such as L-shapes. The position of the collision will be
 * found accurately, but the direction may not be as anticipated as it is based on the <code>CollisionRect</code>s
 * rather than pixel collisions or custom <code>CollisionShape</code>s defined in subclasses of
 * <code>PreciseCollisionGroup</code> If concave sprites are necessary, it might be advisable to break them into groups
...
public abstract class PreciseCollisionGroup extends CollisionGroup {
	...
	/**
	 * The {@link Logger} instance to use for the {@link PreciseCollisionGroup} class.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(PreciseCollisionGroup.class);
	
	/***************************************************************************
	 * This is used to test for non-convergence in pixel perfect collision, or when unusual <code>CollisionShape</code>s
	 * are used. The default value is 0.000001.
	 **************************************************************************/
	protected static double ITERATIVE_BAILOUT = 0.000001;
	
	/***************************************************************************
	 * This is the distance that two objects must be within to be considered adjacent. When a collision occurs, the
	 * <Sprite>s at their reverted positions are guaranteed to be at least this close. This should be larger than
	 * <code>ITERATIVE_BAILOUT</code>. The default value is 0.0001.
	 **************************************************************************/
	 ...
	 * sprites (<code>shape1</code>, <code>shape2</code>) collided.
	 * <p>
	 * 
	...
	public boolean isCollide(Sprite s1, Sprite s2, CollisionShape shape1, CollisionShape shape2) {
		...
		if ((this.pixelPerfectCollision && BasicCollisionGroup.isPixelCollide(s1.getX(), s1.getY(), s1.getImage(),
			...
			// this gets the speed
			double speedX1 = s1.getX() - s1.getOldX(), speedY1 = s1.getY() - s1.getOldY(), speedX2 = s2.getX()
					- s2.getOldX(), speedY2 = s2.getY() - s2.getOldY();
			// now get the bounds for the CollisionShapes
			double x1 = shape1.getX() - speedX1, y1 = shape1.getY() - speedY1, x2 = shape2.getX() - speedX2, y2 = shape2
					.getY() - speedY2;
			double w1 = shape1.getWidth(), h1 = shape1.getHeight(), w2 = shape2.getWidth(), h2 = shape2.getHeight();
			
			if (LOG.isDebugEnabled()) {
				LOG.debug("Collision ({},{}),({},{})-->", new Object[] { s1.getX(), s2.getX(), x1, y1 });
			}
			...
			if (this.checkCollisionHelper(s1, s2, x1, y1, x2, y2, true)) {// collision
				...
				if (speedX1 == 0 && speedY1 == 0 && speedX2 == 0 && speedY2 == 0) {
					...
					if (Math.pow(speedX1, 2) + Math.pow(speedY1, 2) > Math.pow(speedX2, 2) + Math.pow(speedY2, 2)) {// sprite
						// 1
						// faster
						spriteToMove = s1;
					} else {
				...
				if (tHoriz <= tVert) {// X collision happens first
					...
					if (this.checkAdjacencyHelper(s1, s2, x1 + tHoriz * speedX1, y1 + tHoriz * speedY1, x2 + tHoriz
							* speedX2, y2 + tHoriz * speedY2, speedX1, speedY1, speedX2, speedY2, false)) {
					...
					this.collisionSide = yCollision;
					// similar check here
					if (this.checkAdjacencyHelper(s1, s2, x1 + tVert * speedX1, y1 + tVert * speedY1, x2 + tVert
							* speedX2, y2 + tVert * speedY2, speedX1, speedY1, speedX2, speedY2, false)) {
				...
				if (this.checkCollisionHelper(s1, s2, this.collisionX1, this.collisionY1, this.collisionX2,
					...
					if (this.iterativeMethod(s1, s2, 0.0, finalT, x1, y1, x2, y2, speedX1, speedY1, speedX2, speedY2)) {
						...
						if (LOG.isDebugEnabled()) {
							LOG.debug("true: {} ({},{})", new Object[] { this.collisionSide, this.collisionX1,
									this.collisionY1 });
						}
						return true;
					} else {
				...
				else if (this.checkAdjacencyHelper(s1, s2, this.collisionX1, this.collisionY1, this.collisionX2,
						this.collisionY2, speedX1, speedY1, speedX2, speedY2, true)) {
	...
	protected boolean checkAdjacencyHelper(Sprite s1, Sprite s2, double x1, double y1, double x2, double y2,
			double dx1, double dy1, double dx2, double dy2, boolean includePixelPerfect) {
	...
	protected boolean iterativeMethod(Sprite s1, Sprite s2, double lowerT, double higherT, double oldX1, double oldY1,
			double oldX2, double oldY2, double speedX1, double speedY1, double speedX2, double speedY2) {
		// set up working t
		...
		while (true) {
			...
			} else if (this.checkAdjacencyHelper(s1, s2, curX1, curY1, curX2, curY2, speedX1, speedY1, speedX2,
					speedY2, true)) {
